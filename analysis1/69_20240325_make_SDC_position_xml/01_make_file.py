...
x_25pos_lst = [2, 1, 0, -1, -2]
y_25pos_lst = [-2, -1, 0, 1, 2]
xy_25pos_lst = []
position_lst = []
for k in range(len(xy_lst)):
    for j in range(len(y_25pos_lst)):
        for i in range(len(x_25pos_lst)):
            if j % 2 == 0:
                xy_25pos_lst.append([xy_lst[k][0]+x_25pos_lst[i]*interval, xy_lst[k][1]+y_25pos_lst[j]*interval])
            else:
                xy_25pos_lst.append([xy_lst[k][0] + x_25pos_lst[len(x_25pos_lst)-i-1] * interval, xy_lst[k][1] + y_25pos_lst[j] * interval])
...
file1 = open("%s/test_1500pos.txt" % master_folder, 'w')
for k in range(len(xy_25pos_lst)):
    print(k)

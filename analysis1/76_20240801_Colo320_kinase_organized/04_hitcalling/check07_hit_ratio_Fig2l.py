...
for i in range(len(df)):
    ...
    print(temp_lst)
    sum_lst.append(temp_lst[0] + temp_lst[1])
df['sum'] = sum_lst

df_num = df.copy()
df_num = df_num[df_num['hit_sum']<=-1]
df_num = df_num.sort_values(by=['sum', 'hit_sum'], ascending=[True, True])
...
df_num = df.copy()
df_num = df_num[df_num['hit_sum']>=1]
df_num = df_num.sort_values(by=['sum', 'hit_sum'], ascending=[False, False])

...
df_num = df.copy()
df_num = df_num.drop(['group', 'treatment'], axis=1)
...
df_group_seq.to_csv('%s/02_group_seq/01_survival_log2fc_hoechst.txt' % (output_dir), index=False, sep='\t')
...
df_sort = df_sort.drop(['group', 'treatment'], axis=1)
...
# modify sequence
group_seq1 = uti.list_invert(group_seq[:37])+uti.list_invert(group_seq[37:])
df_group_seq = pd.DataFrame({'group': group_seq1})
if not os.path.exists("%s/02_group_seq/" % output_dir):
    os.makedirs("%s/02_group_seq/" % output_dir)
df_group_seq.to_csv('%s/02_group_seq/01_survival_log2fc_hoechst_modified.txt' % (output_dir), index=False, sep='\t')

df_sort = uti.sort_df(df, group_seq1)
df_ctrl = pd.DataFrame()
df_ctrl['ctrl'] = [1 if df_sort['treatment'][i] == 'DMSO' else 0 for i in range(len(df_sort))]
df_sort.index = df_sort['treatment']
df_sort = df_sort.drop(['group', 'treatment'], axis=1)

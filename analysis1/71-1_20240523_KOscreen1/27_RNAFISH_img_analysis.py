...
for i in range(n_img):
    ...
    plt.imsave("%s%s/27_RNAFISH_rep%s_color_img/%s_RNAFISH_img_%s.tiff" % (output_dir, sample, rep, sample, i+1), dis.blending(viewer))
    viewer.close()
    ...
    plt.imsave("%s%s/27_RNAFISH_rep%s_color_img/%s_seg_%s.tiff" % (output_dir, sample, rep, sample, i + 1), dis.blending(viewer))
    viewer.close()
...
if (data['label'].tolist() == data_RNA['label'].tolist()) & (data['fov'].tolist() == data_RNA['fov'].tolist()):
    ...
    for i in range(len(data_RNA)):
        data_temp = data[(data['fov'] == data_RNA['fov'][i]) & (data['label'] == data_RNA['label'][i])].copy().reset_index(drop=True)
        if len(data_temp) == 1:
            pd_temp.loc[len(pd_temp.index)] = [data_temp['hoechst'][0], data_temp['GFP'][0], data_temp['mCherry'][0]]
        else:
            pd_temp.loc[len(pd_temp.index)] = [-1, -1, -1]
            print('wired')
    data_RNA = pd.concat([data_RNA, pd_temp], axis=1)
    print('complete')
    data_RNA.to_csv('%s/%s/27_%s_RNAFISH_rep%s.txt' % (output_dir, sample, sample, rep), index=False, sep='\t')
print("DONE!")
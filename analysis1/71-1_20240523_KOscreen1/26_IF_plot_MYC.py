...
data = pd.read_csv('%s/%s/25_%s_IF_rep%s_red_green_group.txt' % (output_dir, sample, sample, rep), na_values=['.'], sep='\t')
...
fig, ax = plt.subplots(figsize=(4, 6))
fig.subplots_adjust(left=0.4)
sns.swarmplot(data=data_flt, x='group', y='log10_IF', hue_order=hueorder, s=1.5, color=(255 / 255, 140 / 255, 0 / 255))
...
p = ttest_ind(data_flt[data_flt['group'] == 'mCherry']['log10_IF'].tolist(), data_flt[data_flt['group'] == 'GFP']['log10_IF'].tolist())
print(p)

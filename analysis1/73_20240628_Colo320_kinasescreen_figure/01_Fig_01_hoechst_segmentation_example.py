...
for k in ks:
    ...
    for i in range(n_img):
        ...
        if '%s_%s_%s' % (batch, sample, i+1) not in skip['samples'].tolist():
            ...
            img_hoechst = img_crop(skio.imread("%s%s/%s_%s/%s/%s_CH1.tif" % (data_dir, batch, batch, plate, sample, file_name), plugin="tifffile")[:, :, 2], i)
            # print(img_hoechst.shape)
            img_green = img_crop(skio.imread("%s%s/%s_%s/%s/%s_CH2.tif" % (data_dir, batch, batch, plate, sample, file_name), plugin="tifffile")[:, :, 1], i)
            img_red = img_crop(skio.imread("%s%s/%s_%s/%s/%s_CH3.tif" % (data_dir, batch, batch, plate, sample, file_name), plugin="tifffile")[:, :, 0], i)
            img_farred = img_crop(skio.imread("%s%s/%s_%s/%s/%s_CH4.tif" % (data_dir, batch, batch, plate, sample, file_name), plugin="tifffile")[:, :, 0], i)
            ...
            print(img_hoechst.shape)
            img_hoechst_cut = img_hoechst[0:130, 0:130]  # 50uM
            ...
            # viewer.add_image(img_green, blending='additive', colormap='green', contrast_limits=[0, 65535])
            # viewer.add_image(img_red, blending='additive', colormap='red', contrast_limits=[0, 65535])
            viewer.add_image(img_farred, blending='additive', colormap='magenta', contrast_limits=[0, 40000])
            # viewer.add_image(img_seg, blending='additive', contrast_limits=[0, 1])
            if not os.path.exists("%s%s/%s_%s/seg/%s/" % (output_dir, batch, batch, plate, sample)):
                os.makedirs("%s%s/%s_%s/seg/%s/" % (output_dir, batch, batch, plate, sample))
            plt.imsave("%s%s/%s_%s/seg/%s/emiRFP670_%s.tiff" % (output_dir, batch, batch, plate, sample, i + 1),
                       dis.blending(viewer))
            viewer.close()

            viewer = napari.Viewer()
            viewer.add_image(img_hoechst, blending='additive', colormap='blue', contrast_limits=[0, 40000])
            viewer.add_image(img_green, blending='additive', colormap='green', contrast_limits=[0, 40000])
            viewer.add_image(img_red, blending='additive', colormap='red', contrast_limits=[0, 40000])
            # viewer.add_image(img_farred, blending='additive', colormap='magenta', contrast_limits=[0, 65535])
            # viewer.add_image(img_seg, blending='additive', contrast_limits=[0, 1])
            if not os.path.exists("%s%s/%s_%s/seg/%s/" % (output_dir, batch, batch, plate, sample)):
                os.makedirs("%s%s/%s_%s/seg/%s/" % (output_dir, batch, batch, plate, sample))
            plt.imsave("%s%s/%s_%s/seg/%s/FUCCI_%s.tiff" % (output_dir, batch, batch, plate, sample, i + 1),
                       dis.blending(viewer))
            viewer.close()

            viewer = napari.Viewer()
            viewer.add_image(img_hoechst_cut, blending='additive', colormap='blue', contrast_limits=[0, 50000])
            plt.imsave("%s%s/%s_%s/seg/%s/seg_%s_cut.tiff" % (output_dir, batch, batch, plate, sample, i + 1),
                       dis.blending(viewer))
            viewer.close()
            # napari.run()

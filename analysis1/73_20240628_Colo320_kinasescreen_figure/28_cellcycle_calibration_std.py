...
def get_ellipse(x, y, a, b, A, data, data_x, data_y):
    data_flt = data[(((data[data_x] - x) * math.cos(A) + (data[data_y] - y) * math.sin(A)) ** 2) / (a ** 2) + (
                ((data[data_x] - x) * math.sin(A) + (data[data_y] - y) * math.cos(A)) ** 2) / (b ** 2) <= 1].copy()
    return data_flt
...
def get_ellipse1(x, y, a, b, A, data, data_x, data_y):
    ...
    data_flt = data[
        np.sqrt((data[data_x] - x + c * math.cos(A)) ** 2 + (data[data_y] - y + c * math.sin(A)) ** 2) + np.sqrt(
            (data[data_x] - x - c * math.cos(A)) ** 2 + (data[data_y] - y - c * math.sin(A)) ** 2) - 2 * a <= 0].copy()
    return data_flt
...
for s in sample_lst:
    ...
    for i in range(repeat):
        ...
        m = plt.hist(data_temp['log2_H2-3'], bins=50)
        ...
        plt.subplots(figsize=(9, 7))
        m = plt.hist(data_temp['log2_AzaleaB5'], bins=50)
        ...
        print(green_axis)
        plt.close()
        data_G1 = get_ellipse1(12.6, red_axis, 3.6, 0.7, 0, data_temp, 'log2_AzaleaB5', 'log2_H2-3') # 24hr_density
        data_temp = data_temp.drop(data_G1.index)
        data_S = get_ellipse1(green_axis+0.2, 13.2+red_axis-10.1, 2.5, 0.7, math.radians(85), data_temp, 'log2_AzaleaB5', 'log2_H2-3')  # 24hr_density
        data_temp = data_temp.drop(data_S.index)
        data_G2M = get_ellipse1(12+green_axis+0.2-8.3, 14+red_axis-10.1, 3.5, 1.3, math.radians(30), data_temp, 'log2_AzaleaB5', 'log2_H2-3')  # 24hr_density
...
df.to_csv('%s/cell-cycle_sampling_update.txt' % (data_dir), index=False, sep='\t')

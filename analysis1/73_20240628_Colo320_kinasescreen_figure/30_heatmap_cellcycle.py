...
label_seq = pd.read_csv('%s/cluster_label_hoechst_survival_only.txt' % (output_dir), na_values=['.'], sep='\t')
...
for i in range(len(df)):
    df_sort.loc[len(df_sort.index)] = \
        df_flt.iloc[int(label_seq['node'][i])]
...
pd_label.to_csv('%s/cluster_label_hoechst_survival_only.txt' % (output_dir), index=False, sep='\t')"""
...
plt.savefig('%s/heatmap_sort_hoechst_survival_only.pdf' % output_dir)
plt.show()"""

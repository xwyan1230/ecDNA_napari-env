...
for feature in features:
    ...
    ax1 = sns.heatmap(df_drop, cbar=0, linewidths=0.2, vmax=limit, vmin=-limit, square=True,
                      cmap='coolwarm')  # yticklabels=False
    plt.savefig('%s/heatmap_%s_neg_log2_ratio_seq.pdf' % (output_dir, feature))
    plt.show()

    fig, ax = plt.subplots(figsize=(6, 10))
    fig.subplots_adjust(left=0.4)
    ax1 = sns.heatmap(df_drop[:33], cbar=0, linewidths=0.2, vmax=limit, vmin=-limit, square=True,
                      cmap='coolwarm')  # yticklabels=False
    plt.savefig('%s/heatmap_%s_neg_DM_log2_ratio_seq.pdf' % (output_dir, feature))
    plt.show()

    fig, ax = plt.subplots(figsize=(6, 10))
    fig.subplots_adjust(left=0.4)
    ax1 = sns.heatmap(df_drop[250:], cbar=0, linewidths=0.2, vmax=limit, vmin=-limit, square=True,
                      cmap='coolwarm')  # yticklabels=False
    plt.savefig('%s/heatmap_%s_neg_HSR_log2_ratio_seq.pdf' % (output_dir, feature))
    plt.show()

...
df = pd.DataFrame()
folder = '5uM_48hr'
batches = ['5uM_48hr_1', '5uM_48hr_2', '5uM_48hr_3']
for batch in batches:
    data = pd.read_csv('%s%s//%s/%s_cutoff_qc.txt' % (data_dir, folder, batch, batch), na_values=['.'], sep='\t')
    data['batch'] = [batch] * len(data)
    data['group'] = ['%s_%s' % (batch[-1], data['identity'][x]) for x in range(len(data))]

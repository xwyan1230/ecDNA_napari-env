...
features = ['n_G1', 'n_G1S', 'n_S', 'n_G2M', 'n_G2MG1', 'n_neg_G1', 'n_neg_G1S', 'n_neg_S', 'n_neg_G2M', 'n_neg_G2MG1', 'n_pos_G1', 'n_pos_G1S', 'n_pos_S', 'n_pos_G2M', 'n_pos_G2MG1']
...
data_flt = data1[(data1['per_green']<0.05) & (data1['per_red']<0.05)].copy()
...
print(len(data))
data_drop = data.copy()
data_drop = data_drop.drop(data_flt.index)
print(len(data_drop))
...
for k in range(len(features)):
    ...
    for i in data_flt.index:
        if data_flt['rep'][i] == 1:
            df_temp = df_drop[(df_drop['rep1_plate'] == data_flt['plate'][i]) & (df_drop['rep1_sample'] == data_flt['sample'][i])]
            print(list(df_temp.index))
            df_drop = df_drop.drop(df_temp.index)
        else:
            df_temp = df_drop[(df_drop['rep2_plate'] == data_flt['plate'][i]) & (df_drop['rep2_sample'] == data_flt['sample'][i])]
            print(list(df_temp.index))
            df_drop = df_drop.drop(df_temp.index)
    ...
    data_hoechst_hit.to_csv('%s/%s/%s_%s_hit_update1.txt' % (data_dir, folder, folder, feature), index=False, sep='\t')
    ...
    if not os.path.exists('%s/%s/cellcycle/' % (output_dir, folder)):
        os.makedirs('%s/%s/cellcycle/' % (output_dir, folder))
    plt.savefig('%s/%s/cellcycle/%s_%s_rep_wohit_update1.pdf' % (output_dir, folder, folder, feature))
    plt.show()
    ...
    if not os.path.exists('%s/%s/cellcycle/' % (output_dir, folder)):
        os.makedirs('%s/%s/cellcycle/' % (output_dir, folder))
    plt.savefig('%s/%s/cellcycle/%s_%s_rep_perchange_update1.pdf' % (output_dir, folder, folder, feature))
    plt.show()

...
data3 = data[(data['rep'] != 0) & (data['plate'] == batches[2])].copy().reset_index(drop=True)
...
sns.scatterplot(data=df, x=feature, y=feature1, alpha=1, s=20, color='#cccccc')
sns.scatterplot(data=df[df['treatment'] == 'DMSO'], x=feature, y=feature1, alpha=1, s=20, color=rainboo_colors[6])
data_flt = df[((df[feature1]/df[feature]>50) & (df[feature1]<-0.8)) | ((df[feature1]/df[feature]<-50) & (df[feature1]<-0.5))].copy().reset_index(drop=True)
...
# plt.ylim([-6, 1])
# plt.xlim([-6, 1])
print(df[df['treatment']=='XMU-MP-1'][feature])
print(df[df['treatment']=='XMU-MP-1'][feature1])

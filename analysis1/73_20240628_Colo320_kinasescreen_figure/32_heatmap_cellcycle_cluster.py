...
exclude_index1 = label_seq['node'][:25]
...
data_heatmap_scale.index = df.index
data_heatmap_scale = data_heatmap_scale.drop(exclude_index1).reset_index(drop=True)
...
plt.savefig('%s/ahc_cellcycle_only.pdf' % output_dir)
plt.show()
nodes = R['ivl']
...
df = df.drop(exclude_index1).reset_index(drop=True)
...
pd_label.to_csv('%s/cluster_label_cellcycle_only.txt' % (output_dir), index=False, sep='\t')
...
fig.subplots_adjust(left=0.4)
ax1 = sns.heatmap(df_heatmap_sort[::-1], cbar=0, linewidths=0.2, vmax=4, vmin=-4, square=True, cmap='coolwarm', yticklabels=False)  #  yticklabels=False
plt.savefig('%s/heatmap_sort_cellcycle_only.pdf' % output_dir)
plt.show()

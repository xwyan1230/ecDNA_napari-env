...
treatment = 'point5uM_48hr'
plate = 3
save_pos = 3
total_pos = 9

batch1 = os.listdir('%s%s/%s_%s' % (data_dir, treatment, treatment, plate))
batch1 = [k for k in batch1 if '.' not in k] # 59
print(len(batch1))

batch2 = os.listdir('%s%s/%s_%s-1' % (data_dir, treatment, treatment, plate))
batch2 = [k for k in batch2 if '.' not in k]
print(batch2)

for i in range(len(batch2)):
    if batch2[i] == 'XY01':
        source_dir = "%s%s/%s_%s-1/XY01/" % (data_dir, treatment, treatment, plate)
        dest_dir = "%s%s/%s_%s/XY%s/" % (data_dir, treatment, treatment, plate, len(batch1)) if len(batch1) >= 10 else "%s%s/%s_%s/XY0%s/" % (data_dir, treatment, treatment, plate, len(batch1))
        ...
        source_dir = "%s%s/%s_%s-1/%s/" % (data_dir, treatment, treatment, plate, batch2[i])
        dest_folder_number = int(batch2[i][2:])+len(batch1)-1
        print(dest_folder_number)
        dest_dir = "%s%s/%s_%s/XY%s/" % (data_dir, treatment, treatment, plate, dest_folder_number) if dest_folder_number >= 10 else "%s%s/%s_%s/XY0%s/" % (data_dir, treatment, treatment, plate, dest_folder_number)

...
df = pd.read_csv('%ssummary_delta.txt' % data_dir, na_values=['.'], sep='\t')
df_ctrl_GFP = pd.read_csv('%sctrl_GFP.txt' % data_dir, na_values=['.'], sep='\t')
df_ctrl_mCherry = pd.read_csv('%sctrl_mCherry.txt' % data_dir, na_values=['.'], sep='\t')
...
for hue in hue_order:
    for feature in feature_lst:
        ...
        plt.savefig('%s/heatmap_line/%s_%s.pdf' % (output_dir, feature, hue))
        plt.close()
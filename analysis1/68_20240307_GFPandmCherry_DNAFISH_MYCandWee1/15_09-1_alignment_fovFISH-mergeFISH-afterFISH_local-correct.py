...
topleft_final_refine = pd.read_csv('%s/%s/align_topleft_final_manual_%s.txt' % (align_dir, sample, sample), na_values=['.'], sep='\t')

fov = correction_fov-1
...
sample_index = topleft_final_refine[topleft_final_refine['fov'] == correction_fov].index[0]
topleft_final_refine.loc[sample_index] = [sample, correction_fov, topleft_target, min_ratio]
...
print(sample)
print("step09-1 topleft individual FOV local search correction Done!")
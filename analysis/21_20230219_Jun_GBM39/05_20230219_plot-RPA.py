...
for i in range(len(hue_order)):
    ...
    plt.legend()
    plt.savefig('%s/%s_count_RPA_vs_mean_int_DNAFISH_color_by_EdU_%s.pdf' % (output_dir, figure_name, hue_order[i]))
    plt.show()

plt.subplots(figsize=(9, 6))
for i in range(len(hue_order)):
    df_temp = df[df['sample'] == hue_order[i]].copy().reset_index(drop=True)
    sns.scatterplot(data=df_temp, y='total_area_RPA', x='mean_int_DNAFISH', c=df_temp['mean_int_EdU'].tolist(), size=5)
    plt.legend()
    plt.savefig('%s/%s_total_area_RPA_vs_mean_int_DNAFISH_color_by_EdU_%s.pdf' % (output_dir, figure_name, hue_order[i]))
    plt.show()

plt.subplots(figsize=(9, 6))
for i in range(len(hue_order)):
    df_temp = df[df['sample'] == hue_order[i]].copy().reset_index(drop=True)
    sns.scatterplot(data=df_temp, y='mean_int_RPA', x='mean_int_DNAFISH', c=df_temp['mean_int_EdU'].tolist(), size=5)
    plt.legend()
    plt.savefig('%s/%s_mean_int_RPA_vs_mean_int_DNAFISH_color_by_EdU_%s.pdf' % (output_dir, figure_name, hue_order[i]))
    plt.show()

plt.subplots(figsize=(9, 6))
for i in range(len(hue_order)):
    df_temp = df[df['sample'] == hue_order[i]].copy().reset_index(drop=True)
    sns.scatterplot(data=df_temp, y='mean_int_RPA', x='mean_int_DNAFISH', c=df_temp['count_RPA'].tolist(), size=5)
    plt.legend()
    plt.savefig('%s/%s_mean_int_RPA_vs_mean_int_DNAFISH_color_by_count_RPA_%s.pdf' % (output_dir, figure_name, hue_order[i]))
    plt.show()

...
# INPUT PARAMETERS
# file info
master_folder = "/Users/xwyan/Dropbox/LAB/ChangLab/Projects/Data/20230504_analysis_DM_24hr/"
data_dir = "%sdata/" % master_folder
output_dir = "%sfigures/" % master_folder

exp = '20230317_DM_24hr_DNAFISH'
sample = 'G11'
file_name = '%s_%s_%s_RAW' % (exp, sample[0], sample)
...
for fov in range(img_hoechst_stack.shape[0]):
    ...
    tif.imwrite("%s/seg_tif/%s/%s_%s_seg.tif" % (output_dir, sample, sample, fov), img_nuclear_seg_convex)
    ...
    tif.imwrite("%s/seg_tif/%s/%s_%s_ecseg1.tif" % (output_dir, sample, sample, fov), img_DNAFISH_seg1)
    ...
    plt.imsave('%s/color_img/%s/%s_%s_img.tiff' % (output_dir, sample, sample, fov), dis.blending(viewer))
    viewer.close()
    ...
    plt.imsave('%s/color_img/%s/%s_%s_seg.tiff' % (output_dir, sample, sample, fov), dis.blending(viewer))
    viewer.close()

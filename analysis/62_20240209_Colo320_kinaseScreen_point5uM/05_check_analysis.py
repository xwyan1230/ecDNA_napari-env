...
data_flt = data_screen[(data_screen['log2FC_HSR-DM'] < -0.4) | (data_screen['log2FC_HSR-DM'] > 0.35) |
                       (data_screen['n_total'] < 1000) | (data_screen['n_total']-833*data_screen['log2FC_HSR-DM'] < 950) |
                       (data_screen['n_total']+833*data_screen['log2FC_HSR-DM'] < 900)].copy().reset_index(drop=True)
...
# data_woCtrl = data_screen[data_screen['treatment'] != 'DMSO'].copy().reset_index(drop=True)
...
category_lst = []
for i in range(len(data_flt)):
    if (data_flt['log2FC_HSR-DM'][i] < 0) & (data_flt['n_total'][i] < 500):
        category_lst.append('HSR_survival')
    elif data_flt['log2FC_HSR-DM'][i] < 0:
        category_lst.append('HSR')
    elif (data_flt['log2FC_HSR-DM'][i] > 0) & (data_flt['n_total'][i] < 500):

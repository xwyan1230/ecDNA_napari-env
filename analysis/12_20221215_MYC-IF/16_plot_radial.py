...
feature = ['radial_curve_nuclear', 'radial_curve_DNAFISH', 'radial_curve_MYC',
           'radial_curve_normalized']
...
mean_curve3, ci_lower3, ci_higher3 = dat.mean_list(data['radial_curve_MYC'].tolist())
...
for i in range(len(data)):
    plt.plot(x, data['radial_curve_MYC'][i], alpha=0.05, color=[line_colors[2][j]+0.05 for j in range(len(line_colors[2]))])
...
mean_curve4, ci_lower4, ci_higher4 = dat.mean_list(data['radial_curve_normalized'].tolist())
...
heatmap_order = ['radial_curve_normalized', 'radial_curve_MYC']

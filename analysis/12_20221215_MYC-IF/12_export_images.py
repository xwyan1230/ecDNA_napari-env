...
if len(img_hoechst.shape) > 2:
    for i in range(img_hoechst.shape[0]):
        ...
        img_MYC_temp = img_MYC[i, :, :]
        img_hoechst_temp = np.concatenate([np.zeros(shape=[8, 3144]), img_hoechst_temp], axis=0)
        img_DNAFISH_temp = np.concatenate([np.zeros(shape=[8, 3144]), img_DNAFISH_temp], axis=0)
        ...
        viewer.add_image(img_MYC_temp[:3144, :3144], blending='additive', colormap='red', contrast_limits=[0, img_MYC.max()])
        plt.imsave('%s%s/color_img/%s_%s_img_with_MYC.tiff' % (output_dir, sample, sample, i), dis.blending(viewer))
        viewer.close()
else:
    viewer = napari.Viewer()

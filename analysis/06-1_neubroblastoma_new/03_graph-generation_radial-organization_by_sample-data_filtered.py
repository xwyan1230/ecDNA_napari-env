...
fig_size = (12, 9)
# A (12, 9); C (24, 9)
filter = 'total_area_ratio_ecDNA>0.1_data-filtered'
...
data = data_ori[(data_ori['area_nuclear'] > 100) & (data_ori['total_area_ratio_ecDNA'] > 0.1)].copy().reset_index(drop=True)
...
for sample in sample_lst:
    if len(data[data['sample'] == sample]) >= 10:
        print('%s, %s' % (sample, len(data[data['sample'] == sample])))
        sample_lst_filtered.append(sample)

cmap = matplotlib.cm.get_cmap('Spectral')
rgba = [cmap(i) for i in np.arange(0, 1, 1/len(sample_lst_filtered))]
sns.set_palette(sns.color_palette(rgba))

feature = 'radial_distance'
median_lst = []
for sample in sample_lst_filtered:
    temp = data[data['sample'] == sample][feature].median()
    median_lst.append(temp)
sample_lst_sorted = [x for _, x in sorted(zip(median_lst, sample_lst_filtered))]
...
for sample in sample_lst_sorted:
    ...
    number_nuclear = len(data_sample)
    plt.scatter(data_sample['total_area_ratio_ecDNA']**0.5, data_sample['normalized_dis_to_hub_area_v2'],
...
plt.ylim([0, 4])
plt.xlim([0, 1])
plt.savefig('%s/%s-sample-dis_to_hub_area_v2-vs-total_area_ratio_ecDNA_sqrt.pdf' % (save_folder, group))
plt.show()
...
for sample in sample_lst_sorted:
    ...
    _, int_fit_r2_sample, int_fit_a_sample = \
        mat.fitting_linear_b0(list(data_sample['total_area_ratio_ecDNA'] ** 0.5), data_sample['normalized_dis_to_hub_area_v2'].tolist())
...
plt.subplots(figsize=(12, 9))
plt.scatter(df['radial_distance'], df['k_cluster'], color='red', label='group %s, n=%s' % (group, len(sample_lst_filtered)))

...
df = pd.read_csv(("%s%s_n%s_1.txt" % (data_dir2, sample, n_nuclear_convex_dilation)), na_values=['.'], sep='\t')
...
for fov in range(img_mCherry_stack.shape[0]):
    if pos_count == 100:
        ...
        tif.imwrite("%s%s/grid/%s_pos_ecseg1_%s.tif" % (output_dir, sample, sample, img_pos_count), img_mCherry_pos_ecseg)
        ...
        plt.imsave('%s%s/grid/%s_pos_seg1_%s.tiff' % (output_dir, sample, sample, img_pos_count), dis.blending(viewer))
        viewer.close()
    ...
    if neg_count == 100:
        ...
        tif.imwrite("%s%s/grid/%s_neg_ecseg1_%s.tif" % (output_dir, sample, sample, img_neg_count), img_mCherry_neg_ecseg)
        ...
        plt.imsave('%s%s/grid/%s_neg_seg1_%s.tiff' % (output_dir, sample, sample, img_neg_count), dis.blending(viewer))
        viewer.close()
    ...
    img_ecseg = skio.imread("%s%s/seg_tif/%s/%s_%s_ecseg1_n12.tif" % (data_dir2, sample, batch, sample, fov),
                            plugin="tifffile")

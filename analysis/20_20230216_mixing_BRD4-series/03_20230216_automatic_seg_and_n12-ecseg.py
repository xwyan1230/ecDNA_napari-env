...
for f in range(img_mCherry_stack.shape[0]):
    ...
    img_laminB = img_laminB_stack[fov, :, :]
    ...
    if not os.path.exists("%s%s/seg_tif/%s/" % (output_dir, sample, batch)):
        os.makedirs("%s%s/seg_tif/%s/" % (output_dir, sample, batch))
    tif.imwrite("%s%s/seg_tif/%s/%s_%s_seg.tif" % (output_dir, sample, batch, sample, fov), img_nuclear_seg_convex)
    ...
    tif.imwrite("%s%s/seg_tif/%s/%s_%s_ecseg_n%s.tif" % (output_dir, sample, batch, sample, fov, n_nuclear_convex_dilation), img_DNAFISH_seg)

    if not os.path.exists("%s%s/color_img/%s/" % (output_dir, sample, batch)):
        os.makedirs("%s%s/color_img/%s/" % (output_dir, sample, batch))

    viewer = napari.Viewer()
    viewer.add_image(img_nuclear, blending='additive', colormap='blue', contrast_limits=[0, img_nuclear.max()])
    viewer.add_image(img_DNAFISH, blending='additive', colormap='green', contrast_limits=[0, img_DNAFISH.max()])
    plt.imsave('%s%s/color_img/%s/%s_%s_img.tiff' % (output_dir, sample, batch, sample, fov), dis.blending(viewer))
    viewer.close()
    ...
    viewer.add_image(img_DNAFISH, blending='additive', colormap='green', contrast_limits=[0, img_DNAFISH.max()])
    plt.imsave('%s%s/color_img/%s/%s_%s_img_with_mCherry.tiff' % (output_dir, sample, batch, sample, fov), dis.blending(viewer))
    viewer.close()
    ...
    plt.imsave('%s%s/color_img/%s/%s_%s_seg_n%s.tiff' % (output_dir, sample, batch, sample, fov, n_nuclear_convex_dilation), dis.blending(viewer))
    viewer.close()

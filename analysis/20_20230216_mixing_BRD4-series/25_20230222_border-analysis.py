...
import pandas as pd
from skimage.morphology import disk, dilation, medial_axis, binary_dilation, binary_erosion
from skimage.measure import label, regionprops
import shared.image as ima
import matplotlib.pyplot as plt
import numpy as np
import shared.dataframe as dat
import shared.display as display
...
data = pd.DataFrame(columns=['nuclear', 'FOV', 'n_nuclear_convex_dilation', 'batch',
                             'centroid_nuclear', 'area_nuclear', 'circ_nuclear', 'mean_int_nuclear', 'mCherry_mean',
                             'mean_int_DNAFISH', 'n_ecDNA', 'perimeter_ecDNA',
                             ...
                             'cum_int_ind_ecDNA', 'cum_int_n_half', 'mean_int_border0', 'mean_int_border_m1', 'mean_int_border_m2', 'mean_int_border_p1', 'mean_int_border_p2'])
...
for fov in range(img_hoechst_stack.shape[0]):
    ...
    img_ecSeg = skio.imread("%s%s/seg_tif/%s/%s_%s_ecseg1_n12.tif" % (data_dir2, sample, batch, sample, fov), plugin="tifffile")
    ...
    for i in range(len(nuclear_props)):
        ...
        local_DNAFISH_seg_border = local_DNAFISH_seg.copy()
        local_DNAFISH_seg_border_m1 = binary_erosion(local_DNAFISH_seg)
        local_DNAFISH_seg_border_m2 = binary_erosion(local_DNAFISH_seg_border_m1)
        local_DNAFISH_seg_border_m3 = binary_erosion(local_DNAFISH_seg_border_m2)
        local_DNAFISH_seg_border_p1 = binary_dilation(local_DNAFISH_seg_border)
        local_DNAFISH_seg_border_p2 = binary_dilation(local_DNAFISH_seg_border_p1)
        local_DNAFISH_seg_border_p2[local_DNAFISH_seg_border_p1 == 1] = 0
        local_DNAFISH_seg_border_p2[local_nuclear_seg == 0] = 0
        local_DNAFISH_seg_border_p1[local_DNAFISH_seg_border == 1] = 0
        local_DNAFISH_seg_border_p2[local_nuclear_seg == 0] = 0
        local_DNAFISH_seg_border[local_DNAFISH_seg_border_m1 == 1] = 0
        local_DNAFISH_seg_border_m1[local_DNAFISH_seg_border_m2 == 1] = 0
        local_DNAFISH_seg_border_m2[local_DNAFISH_seg_border_m3 == 1] = 0

        """if not os.path.exists("%s%s/border_img/%s/" % (output_dir, sample, batch)):
            os.makedirs("%s%s/border_img/%s/" % (output_dir, sample, batch))

        viewer = napari.Viewer()
        viewer.add_image(local_nuclear, blending='additive', colormap='blue', contrast_limits=[0, img_nuclear_bgc.max()])
        viewer.add_image(local_DNAFISH, blending='additive', colormap='green', contrast_limits=[0, img_DNAFISH_bgc.max()])
        plt.imsave('%s%s/border_img/%s/%s_%s_%s_img.tiff' % (output_dir, sample, batch, sample, fov, i), display.blending(viewer))
        viewer.close()

        viewer = napari.Viewer()
        viewer.add_image(local_nuclear_seg, blending='additive', colormap='blue', contrast_limits=[0, img_nuclear_bgc.max()])
        viewer.add_image(local_DNAFISH_seg_border_m2, blending='additive', colormap='gray',contrast_limits=[0, 1])
        viewer.add_image(local_DNAFISH_seg_border_m1, blending='additive', colormap='blue', contrast_limits=[0, 1])
        viewer.add_image(local_DNAFISH_seg_border, blending='additive', colormap='magenta', contrast_limits=[0, 1])
        viewer.add_image(local_DNAFISH_seg_border_p1, blending='additive', colormap='green', contrast_limits=[0, 1])
        viewer.add_image(local_DNAFISH_seg_border_p2, blending='additive', colormap='red', contrast_limits=[0, 1])
        plt.imsave('%s%s/border_img/%s/%s_%s_%s_seg.tiff' % (output_dir, sample, batch, sample, fov, i), display.blending(viewer))
        viewer.close()"""

        # basic measurements
        mean_int_border = np.sum(local_DNAFISH_seg_border * local_DNAFISH)/np.sum(local_DNAFISH_seg_border)
        mean_int_border_m1 = np.sum(local_DNAFISH_seg_border_m1 * local_DNAFISH) / np.sum(local_DNAFISH_seg_border_m1)
        mean_int_border_m2 = np.sum(local_DNAFISH_seg_border_m2 * local_DNAFISH) / np.sum(local_DNAFISH_seg_border_m2)
        mean_int_border_p1 = np.sum(local_DNAFISH_seg_border_p1 * local_DNAFISH) / np.sum(local_DNAFISH_seg_border_p1)
        mean_int_border_p2 = np.sum(local_DNAFISH_seg_border_p2 * local_DNAFISH) / np.sum(local_DNAFISH_seg_border_p2)
        ...
        perimeter_ecDNA = np.sum(local_DNAFISH_seg_border)
        ...
        data.loc[len(data.index)] = [i, fov, n_nuclear_convex_dilation, batch,
                                     ...
                                     cum_area_ind_ecDNA, cum_area_n_half,
                                     cum_area_ratio_ind_ecDNA, cum_area_ratio_n_half,
                                     cum_int_ind_ecDNA, cum_int_n_half, mean_int_border, mean_int_border_m1, mean_int_border_m2, mean_int_border_p1, mean_int_border_p2]
...
if batch > 0:
    data.to_csv('%s%s_%s_n%s_1.txt' % (output_dir, sample, batch, n_nuclear_convex_dilation), index=False, sep='\t')
else:
    data.to_csv('%s%s_n%s_1.txt' % (output_dir, sample, n_nuclear_convex_dilation), index=False, sep='\t')

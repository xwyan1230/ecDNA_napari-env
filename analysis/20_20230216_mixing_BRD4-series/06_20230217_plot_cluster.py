...
# data filter
# df_sort = df_sample
df_sort = df_sample[df_sample['total_area_ecDNA_sqrt_normalized'] > 0.15].copy().reset_index(drop=True)
...
for i in range(len(hue_order)):
    data_sample = df_sort[df_sort['sample'] == hue_order[i]].copy().reset_index(drop=True)
    mean_dis.append(np.mean(data_sample['dis_to_hub_area_normalized'].tolist()))
    std_dis.append(np.std(data_sample['dis_to_hub_area_normalized'].tolist()))

print("mean")
print(mean_dis)
plt.subplots(figsize=(4, 9))
fig.subplots_adjust(left=0.2)
plt.bar(hue_order, mean_dis, color=line_colors)
plt.savefig('%s%s_dis_mean.pdf' % (output_dir, figure_name))
plt.ylabel('mean_d')
plt.show()
...
plt.subplots(figsize=(4, 9))
fig.subplots_adjust(left=0.2)
plt.bar(hue_order, std_dis, color =line_colors)
plt.savefig('%s%s_dis_std.pdf' % (output_dir, figure_name))
plt.ylabel('std_d')
plt.show()
...
for f in feature:
    ...
    for k in range(len(hue_order)):
        ...
        for i in range(len(data)):
            plt.plot(data[f][i], alpha=0.01, color=[line_colors[k][j] + 0.05 for j in range(len(line_colors[k]))])

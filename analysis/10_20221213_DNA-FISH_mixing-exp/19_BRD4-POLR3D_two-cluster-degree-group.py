...
mCherry_pos_threshold = 0.75
mCherry_neg_threshold = 0.6
feature = 'dis_to_hub_area_normalized'
...
for fov in range(img_mCherry_stack.shape[0]):
    ...
    for i in range(len(mCherry_props)):
        if df[(df['FOV'] == fov) & (df['nuclear'] == i)][feature].tolist()[0] > mCherry_pos_threshold:
            img_mCherry_pos[img_seg == mCherry_props[i].label] = mCherry_props[i].label
        elif df[(df['FOV'] == fov) & (df['nuclear'] == i)][feature].tolist()[0] < mCherry_neg_threshold:
    ...
    plt.imsave('%s%s/color_img/%s_%s_img_label_%s.tiff' % (output_dir, sample, sample, fov, feature), dis.blending(viewer))
    viewer.close()
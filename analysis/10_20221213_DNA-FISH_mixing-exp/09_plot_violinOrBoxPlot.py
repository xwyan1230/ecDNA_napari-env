...
data_dir = "%sfigures/txt/" % master_folder
output_dir = "%sfigures/" % master_folder

df = pd.read_csv(("%sDM_mix_DM-H2B-mCherry_ecDNA.txt" % data_dir), na_values=['.'], sep='\t')
df['cluster_degree'] = df['dis_to_hub_area']/df['total_area_ecDNA']
...
for i in range(len(df)):
    if df['mCherry_mean'].tolist()[i] < 4800:
        sample.append('neg')
...
sns.boxplot(data=df_sort, x='sample', y='total_area_ratio_ecDNA', showfliers = False)
plt.savefig('%s/total_area_ratio_ecDNA.pdf' % output_dir)
plt.show()

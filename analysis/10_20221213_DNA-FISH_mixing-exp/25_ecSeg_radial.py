...
img_FISH_stack = skio.imread("%s%s_ch01.tif" % (data_dir1, file_name), plugin="tifffile")
...
for fov in range(img_mCherry_stack.shape[0]):
    ...
    tif.imwrite("%s%s/seg_tif/%s_%s_ecseg_n12.tif" % (output_dir, sample, sample, fov), img_DNAFISH_seg)
    ...
    plt.imsave('%s%s/color_img/%s_%s_seg_n12.tiff' % (output_dir, sample, sample, fov), dis.blending(viewer))
    viewer.close()

...
total_fov = 8
start_fov = 1
bg_DNAFISH_proj = 8903.276881858985
...
for i in range(len(df)):
    ...
    for j in range(15):
        ...
        local_DNAFISH_seg = obj.remove_small(local_DNAFISH_seg, 6)
        local_DNAFISH_seg[local_nuclear_seg == 0] = 0
        ecDNA_props = regionprops(label(local_DNAFISH_seg), local_DNAFISH)
...
data.to_csv('%s%s/%s_n%s.txt' % (output_dir, sample, sample, n_nuclear_convex_dilation), index=False, sep='\t')

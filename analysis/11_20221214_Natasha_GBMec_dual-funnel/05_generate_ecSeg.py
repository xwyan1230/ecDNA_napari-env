...
for f in range(total_fov):
    ...
    if not os.path.exists("%s%s/seg_z/" % (output_dir, sample)):
        os.makedirs("%s%s/seg_z/" % (output_dir, sample))
    tif.imwrite("%s%s/seg_z/%s_%s_ecSeg_z.tif" % (output_dir, sample, sample, fov), im_z_stack_DNAFISH_seg_z)
    tif.imwrite("%s%s/seg_z/%s_%s_seg_z.tif" % (output_dir, sample, sample, fov), im_z_stack_nuclear_seg_z)
    ...
    plt.imsave('%s%s/seg_z/%s_%s_napari-img.tiff' % (output_dir, sample, sample, fov), dis.blending(viewer))
    viewer.close()
    ...
    plt.imsave('%s%s/seg_z/%s_%s_napari-seg.tiff' % (output_dir, sample, sample, fov), dis.blending(viewer))
    viewer.close()

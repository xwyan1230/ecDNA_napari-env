...
for f in range(total_fov):
    ...
    for i in range(len(shapes.data)):
        ...
        img_for_ecDNA_seg = cell_FISH.copy()
        img_mask_for_ecDNA_seg = cell_FISH_mask.copy()

        # identify local maxima in ecDNA channel
        ecDNA_max = img_mask_for_ecDNA_seg.max()
        maxima_value = 4000
        maxima = extrema.h_maxima(img_for_ecDNA_seg, maxima_value)
        maxima_outside_chromosome = maxima.copy()
        maxima_outside_chromosome[chromosome_seg_filter == 1] = 0
        elevation_map = sobel(img_for_ecDNA_seg)

        # identify ecDNA outside of chromosome
        markers = np.zeros_like(img_mask_for_ecDNA_seg)
        markers[img_mask_for_ecDNA_seg < (ecDNA_max / 5)] = 1  # 2500
        markers[maxima_outside_chromosome == 1] = 2
        ecDNA_seg_outsideChromosome = watershed(elevation_map, markers)
        ecDNA_seg_outsideChromosome[chromosome_seg_filter == 1] = 0
        ecDNA_seg_outsideChromosome[img_mask_for_ecDNA_seg == 0] = 0
        ecDNA_seg_outsideChromosome_label = obj.label_remove_small(label(ecDNA_seg_outsideChromosome, connectivity=1), 7)
        ecDNA_seg_outsideChromosome_label = obj.label_remove_large(ecDNA_seg_outsideChromosome_label, 100)
        tif.imwrite("%sindividual_cell_tif/%s_%s_%s_chr_seg.tif" % (output_dir, sample, fov, nuclear), chromosome_seg)

        # get ecDNA centroid and create ecDNA mask
        ecDNA_props_temp = regionprops(ecDNA_seg_outsideChromosome_label, cell_FISH)
        ecDNA_centroid = [ecDNA_props_temp[i].centroid for i in range(len(ecDNA_props_temp))]

        ecDNA_seg = np.zeros_like(cell_FISH)
        for j in ecDNA_centroid:
            ecDNA_seg[int(j[0]), int(j[1])] = 1
        ecDNA_seg[chromosome_seg_filter == 1] = 0
        ecDNA_seg[cell_FISH_mask == 0] = 0
        tif.imwrite("%sindividual_cell_tif/%s_%s_%s_ecDNA_seg_by_FISH.tif" % (output_dir, sample, fov, nuclear),
                    ecDNA_seg)

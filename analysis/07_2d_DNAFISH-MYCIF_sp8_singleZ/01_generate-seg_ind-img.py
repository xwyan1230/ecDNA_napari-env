...
for f in range(total_fov):
    ...
    tif.imwrite("%s%s/%s_ecSeg.tif" % (master_folder, sample, file_prefix), img_DNAFISH_seg)
    ...
    plt.imsave('%s%s/%s_napari-seg.tiff' % (save_path, sample, file_prefix), dis.blending(viewer1))
    viewer1.close()

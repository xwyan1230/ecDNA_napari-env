...
sample_prefix = ''
version = 2
save_folder = "%sv4_figures/%s_WT-Ctrl2/" % (master_folder, sample)
if not os.path.exists(save_folder):
    os.makedirs(save_folder)

# default setting
control = ['DM_Jun_Ctrl1']
WT = ['DM_Jun_Ctrl2']
...
# LOAD FILE
data_sample_ori = pd.read_csv('%s%s_v%s.txt' % (master_folder, sample, version), na_values=['.'], sep='\t')
data_control1_ori = pd.read_csv('%s%s_v%s.txt' % (master_folder, control[0], version), na_values=['.'], sep='\t')
data_WT_ori = pd.read_csv('%s%s_v%s.txt' % (master_folder, WT[0], version), na_values=['.'], sep='\t')

data_sample = data_sample_ori[data_sample_ori['mean_int_ecDNA'] > 5000].copy().reset_index()
print(len(data_sample_ori[data_sample_ori['mean_int_ecDNA'] <= 5000]))
data_control1 = data_control1_ori[data_control1_ori['mean_int_ecDNA'] > 5000].copy().reset_index()
print(len(data_control1_ori[data_control1_ori['mean_int_ecDNA'] <= 5000]))
data_WT = data_WT_ori[data_WT_ori['mean_int_ecDNA'] > 5000].copy().reset_index()
print(len(data_WT_ori[data_WT_ori['mean_int_ecDNA'] <= 5000]))
...
feature = ['area_nuclear',
           ...
           'cum_area_n_half', 'cum_area_ratio_n_half', 'cum_int_n_half']
...
feature_x_lst = ['mean_int_ecDNA', 'total_int_ecDNA', 'total_area_ecDNA', 'total_area_ratio_ecDNA', 'mean_int_DNAFISH',
                 'total_int_DNAFISH']
feature_y_lst = ['mean_int_IF', 'total_int_IF', 'total_area_ecDNA', 'total_area_ratio_ecDNA', 'total_int_nuclear']
...
for f in feature:
    ...
    mean_curve3, ci_lower3, ci_higher3 = dat.mean_list(data_control1[f].tolist())

    plt.subplots(figsize=(6, 4))
    for i in range(len(data_sample)):
        plt.plot(x, data_sample[f][i][1:r_range], alpha=0.01, color=[line_colors[0][j] + 0.05 for j in range(len(line_colors[0]))])
    for i in range(len(data_WT)):
        plt.plot(x, data_WT[f][i][1:r_range], alpha=0.01, color=[line_colors[1][j]+0.05 for j in range(len(line_colors[1]))])
    for i in range(len(data_control1)):
        plt.plot(x, data_control1[f][i][1:r_range], alpha=0.01, color=[line_colors[2][j]+0.05 for j in range(len(line_colors[2]))])
    plt.plot(x, mean_curve1[1:r_range], color=line_colors[0], label='%s, n=%s' % (sample, number_nuclear1))
    plt.plot(x, mean_curve2[1:r_range], color=line_colors[1], label='%s, n=%s' % ('WT', number_nuclear2))
    plt.plot(x, mean_curve3[1:r_range], color=line_colors[2], label='%s, n=%s' % ('Ctrl', number_nuclear3))
    plt.plot(x, ci_lower1[1:r_range], color=line_colors[0], linestyle='--', linewidth=0.5)
    plt.plot(x, ci_higher1[1:r_range], color=line_colors[0], linestyle='--', linewidth=0.5)
    plt.plot(x, ci_lower2[1:r_range], color=line_colors[1], linestyle='--', linewidth=0.5)
    plt.plot(x, ci_higher2[1:r_range], color=line_colors[1], linestyle='--', linewidth=0.5)
    plt.plot(x, ci_lower3[1:r_range], color=line_colors[2], linestyle='--', linewidth=0.5)
    plt.plot(x, ci_higher3[1:r_range], color=line_colors[2], linestyle='--', linewidth=0.5)
    plt.xlabel(x_label)
    plt.ylabel(f)
    plt.legend()
    plt.savefig('%s/%s_%s.pdf' % (save_folder, f, sample))

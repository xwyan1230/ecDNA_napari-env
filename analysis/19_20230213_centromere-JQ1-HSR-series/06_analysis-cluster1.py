...
data_dir1 = "%sdata/20221111_Tel-CENPB/" % master_folder
...
img_CENPB_stack = skio.imread("%s%s_ch03.tif" % (data_dir1, file_name), plugin="tifffile")
...
for fov in range(img_hoechst_stack.shape[2]):
    print("Analyzing %s, fov %s" % (sample, fov))
    img_nuclear_bgc = img_hoechst_stack[:, :, fov]
    img_DNAFISH_bgc = img_CENPB_stack[:, :, fov]
    ...
    for i in range(len(nuclear_props)):
        ...
        data.loc[len(data.index)] = [i, fov, n_nuclear_convex_dilation,
                                     ...
                                     radial_distribution_relative_r_normalized,
                                     radial_distribution_relative_r_DNAFISH_seg, nuclear_int,
                                     DNAFISH_int, DNAFISH_seg_label, int_r_to_edge, int_r_to_center, int_relative_r,
                                     g, g0, g_value,

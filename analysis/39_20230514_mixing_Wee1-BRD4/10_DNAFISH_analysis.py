...
data = pd.DataFrame(columns=['nuclear', 'batch', 'FOV', 'n_nuclear_convex_dilation',
...
# for fov in range(60):
    ...
    for i in range(len(nuclear_props)):
        ...
        data.loc[len(data.index)] = [i, batch, fov, n_nuclear_convex_dilation,
...
data.to_csv('%s%s_%s_n%s.txt' % (output_dir, sample, batch, n_nuclear_convex_dilation), index=False, sep='\t')

...
data = pd.DataFrame(columns=['nuclear', 'batch', 'FOV', 'label',
                             'n_nuclear_convex_dilation',
                             'centroid_nuclear', 'area_nuclear', 'circ_nuclear', 'mean_int_nuclear',
                             'mean_int_DNAFISH', 'n_ecDNA',
                             'mean_int_ecDNA', 'area_ecDNA_individual', 'area_ratio_ecDNA_individual', 'mean_int_ecDNA_individual', 'total_area_ecDNA', 'total_area_ratio_ecDNA',
                             'dis_to_hub_area',
...
# for fov in range(60):
    ...
    img_seg = skio.imread("%s%s/seg_tif/%s_%s_%s_seg.tif" % (data_dir1, sample, sample, batch, fov), plugin="tifffile")
    img_ecSeg = skio.imread("%s%s/seg_tif/%s_%s_%s_ecseg1.tif" % (data_dir1, sample, sample, batch, fov), plugin="tifffile")
    ...
    for i in range(len(nuclear_props)):
        ...
        data.loc[len(data.index)] = [i, batch, fov, label_nuclear,
                                     n_nuclear_convex_dilation,
...
data.to_csv('%s%s_%s_n%s_new.txt' % (output_dir, sample, batch, n_nuclear_convex_dilation), index=False, sep='\t')

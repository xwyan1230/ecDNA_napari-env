...
for sample in sample_lst:
    ...
    """data_z_full = pd.read_csv('%s%s/%s/%s_z.txt' % (master_folder, sample[0], sample[1:], sample), na_values=['.'],
                              sep='\t')
    data_z_full['centroid_nuclear'] = [dat.str_to_float(data_z_full['centroid_nuclear'][i]) for i in
                                       range(len(data_z_full))]
    ...
    for i in range(len(data_z)):
        ...
        if weight != 0:
            ...
            local_IF_props = regionprops(label(local_nuclear_seg_convex), local_IF)
            ...
            angle_distribution_DNAFISH_bg_correct_smooth = \
                dat.list_circle_smooth(angle_distribution_DNAFISH_bg_correct, 7)

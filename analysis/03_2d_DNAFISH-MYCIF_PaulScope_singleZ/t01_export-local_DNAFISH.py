...
for i in range(len(data_z)):
    ...
    if weight != 0:
        ...
        plt.imsave('%s%s/%s/%s/%s_DNAFISH_fov%s_z%s_i%s.tiff' % (master_folder, sample_row, sample_num, save_folder,
                                                                 sample, fov, z_current, label_nuclear), local_DNAFISH,
                   vmin=0, vmax=6000)
        plt.imsave('%s%s/%s/%s/%s_ecSeg_fov%s_z%s_i%s.tiff' % (master_folder, sample_row, sample_num, save_folder,
                                                               sample, fov, z_current, label_nuclear), ecDNA_seg,
                   vmin=0, vmax=1)

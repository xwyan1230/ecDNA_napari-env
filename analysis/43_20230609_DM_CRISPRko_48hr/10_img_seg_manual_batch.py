...
for f in range(total_fov):
    ...
    img_nuclear_seg = skio.imread("%s/seg/%s/seg_tif/%s_%s_%s_seg.tif" % (data_dir, sample, sample, batch, fov), plugin="tifffile")
    img_ecDNA_seg = skio.imread("%s/seg/%s/seg_tif/%s_%s_%s_ecseg1.tif" % (data_dir, sample, sample, batch, fov), plugin="tifffile")
    ...
    tif.imwrite("%sseg1/%s/seg_tif/%s_%s_%s_seg.tif" % (output_dir, sample, sample, batch, fov), img_nuclear_seg_convex)
    ...
    plt.imsave('%sseg1/%s/color_img/%s_%s_%s_img.tiff' % (output_dir, sample, sample, batch, fov), dis.blending(viewer))
    viewer.close()
    ...
    plt.imsave('%sseg1/%s/color_img/%s_%s_%s_seg.tiff' % (output_dir, sample, sample, batch, fov), dis.blending(viewer))
    napari.run()

...
for f in range(total_fov):
    ...
    for i in range(len(nuclear_props)):
        ...
        for j in range(15):
            ...
            # viewer.add_image(local_nuclear_seg, blending='additive')
            # viewer.add_image(local_DNAFISH_seg, blending='additive')
            # napari.run()
...
data.to_csv('%s%s/%s_n%s_updated1_%s.txt' % (output_dir, sample, sample, n_nuclear_convex_dilation, batch), index=False, sep='\t')

...
for f in range(total_fov):
    ...
    img_seg = skio.imread("%s/seg/%s/seg_tif/%s_%s_seg.tif" % (data_dir, sample, sample, fov),
                                  plugin="tifffile")
    img_ecSeg = skio.imread("%s/seg/%s/seg_tif/%s_%s_ecseg1.tif" % (data_dir, sample, sample, fov),
                                plugin="tifffile")
...
data.to_csv('%s%s/%s_n%s_simplified.txt' % (output_dir, sample, sample, n_nuclear_convex_dilation), index=False, sep='\t')

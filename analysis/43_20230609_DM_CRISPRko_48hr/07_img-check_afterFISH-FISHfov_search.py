...
fov = 0
# filename = '20230601_CRISPRko_48hr_DNAFISH_%s_%s_s00_ch01' % (row, sample)
filename = '20230602_DM_CRISPRko_48hr_DNAFISH_%s_%s_s00_ch01' % (row, sample)
...
print(s)
topleft_target = [img_after_hoechst.shape[0]-s-250-border, 250+border]
...
for fov in range(total_fov):
    print(fov)
    if fov < 10:
        # filename = '20230601_CRISPRko_48hr_DNAFISH_%s_%s_s0%s_ch01' % (row, sample, fov)
        filename = '20230602_DM_CRISPRko_48hr_DNAFISH_%s_%s_s0%s_ch01' % (row, sample, fov)
    else:
        # filename = '20230601_CRISPRko_48hr_DNAFISH_%s_%s_s%s_ch01' % (row, sample, fov)
        filename = '20230602_DM_CRISPRko_48hr_DNAFISH_%s_%s_s%s_ch01' % (row, sample, fov)
    img_hoechst = skio.imread("%s/FISH/%s/%s.tif" % (data_dir, sample, filename), plugin="tifffile")
    ori_shape = img_hoechst.shape
    ...
    data.to_csv('%s%s/%s_alignment.txt' % (output_dir, sample, sample), index=False, sep='\t')
...
# viewer.add_image(img_hoechst_DNAFISH_cut, blending='additive', contrast_limits=[0, 65535])
viewer.add_image(img_hoechst_local, blending='additive', colormap='green', contrast_limits=[0, 65535])
plt.imsave("%s/%s/%s_alignment_local.tiff" % (output_dir, sample, sample), dis.blending(viewer))
viewer.close()
...
df_WT = pd.read_csv('%s%s_n%s_WT.txt' % (data_dir, exp, n_nuclear_convex_dilation), na_values=['.'], sep='\t')
...
for i in range(len(plates)):
    plate = plates[i]
    df = pd.read_csv('%s%s_n%s.txt' % (data_dir, plate, n_nuclear_convex_dilation), na_values=['.'], sep='\t')

...
for f in range(total_fov):
    ...
    for i in range(len(shapes.data)):
        ...
        maxima_value = int(FISH_max_outside_chromosome * 20 / bg_FISH)
        maxima = extrema.h_maxima(cell_FISH_mask, maxima_value)
        maxima[sub_masks == 0] = 0
        maxima[chromosome_seg_filter == 1] = 0
        elevation_map = sobel(cell_FISH_mask)

        # identify signal by local
        local = threshold_local(cell_FISH, 9)
        FISH_seg_local = cell_FISH_mask > local
        FISH_seg_local = binary_erosion(FISH_seg_local)
        FISH_seg_local = binary_erosion(FISH_seg_local)
        FISH_seg_local_label = obj.label_resort(obj.label_remove_small(label(FISH_seg_local), 2))
        FISH_seg_local_label = dilation(FISH_seg_local_label)

        # identify signal by watershed
        ...
        markers[maxima == 1] = 2
        FISH_seg_watershed = watershed(elevation_map, markers)
        FISH_seg_watershed_label = obj.label_remove_small(label(FISH_seg_watershed), 2)
        # FISH_seg_watershed_label = obj.label_resort(obj.label_remove_large(label(FISH_seg_watershed), 200))

        # measure mean FISH intensity
        FISH_seg_watershed = np.zeros_like(cell_FISH)
        FISH_seg_watershed[FISH_seg_watershed_label > 0] = 1
        mean_int_FISH_DNA = np.sum(cell_FISH_bg_corrected * FISH_seg_watershed) / np.sum(FISH_seg_watershed)

        # generate total FISH segmentation
        FISH_seg_total_label = FISH_seg_local_label.copy()
        count = FISH_seg_local_label.max() + 1
        FISH_seg_watershed_props = regionprops(FISH_seg_watershed_label, FISH_seg_local_label)
        for j in range(len(FISH_seg_watershed_props)):
            seg_in_local = list(set(FISH_seg_watershed_props[j].image_intensity.flatten()))
            seg_in_local = [x for x in seg_in_local if x != 0]
            if len(seg_in_local) == 0:
                FISH_seg_total_label[FISH_seg_watershed_label == FISH_seg_watershed_props[j].label] = count
                count = count + 1
            elif len(seg_in_local) == 1:
                FISH_seg_total_label[FISH_seg_watershed_label == FISH_seg_watershed_props[j].label] = seg_in_local[0]
        ...
        FISH_seg_total_edge_props_table = pd.DataFrame(regionprops_table(FISH_seg_total_label_edge, cell_FISH_bg_corrected,
                                                                         properties=['label', 'intensity_mean']))
        ...
        for j_edge in range(FISH_seg_total_label_edge.max()):
            ...
            if (mean_int_j > (mean_int_FISH_DNA * 0.8)) & (mean_int_j > (mean_int_j_edge * 1.5)):

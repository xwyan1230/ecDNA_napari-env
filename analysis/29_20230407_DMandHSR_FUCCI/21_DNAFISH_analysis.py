...
img_hoechst_stack = skio.imread("%s%s/%s_ch01.tif" % (data_dir, sample, file_name), plugin="tifffile")
img_DNAFISH_stack = skio.imread("%s%s/%s_ch00.tif" % (data_dir, sample, file_name), plugin="tifffile")
...
for fov in range(img_hoechst_stack.shape[0]):
    ...
    img_seg = skio.imread("%s%s/seg_tif/%s_%s_seg.tif" % (data_dir, sample, sample, fov), plugin="tifffile")
    img_ecSeg = skio.imread("%s%s/seg_tif/%s_%s_ecseg1_n12.tif" % (data_dir, sample, sample, fov), plugin="tifffile")

...
for i in range(len(df_sort)):
    if (df_sort['ln_total_int_MYC'][i] < float(hue_order[0])-0.125) | (df_sort['ln_total_int_MYC'][i] >= float(hue_order[-1])+0.125):
        MYC_group.append(0)
    else:
        for j in range(len(hue_order)):
            if (df_sort['ln_total_int_MYC'][i] >= float(hue_order[j])-0.125) & (df_sort['ln_total_int_MYC'][i] < float(hue_order[j])+0.125):
                MYC_group.append(j+1)
                break
df_sort['MYC_group'] = MYC_group

MYC_group1 = []
for i in range(len(df_sort)):
    if df_sort['ln_total_int_MYC'][i] < 17:
        MYC_group1.append(1)
    elif df_sort['ln_total_int_MYC'][i] < 18:
        MYC_group1.append(2)
    elif df_sort['ln_total_int_MYC'][i] < 19:
        MYC_group1.append(3)
    else:
        MYC_group1.append(5)
df_sort['MYC_group1'] = MYC_group1
...
sinaplot(data=df_sort, x='MYC_group1', y='dis_to_hub_area_normalized', order=hue_order2, violin=False, scale='area')
# plt.savefig('%s/%s_dis_to_hub_area_normalized.pdf' % (output_dir, figure_name))
plt.show()

quantiles2 = np.quantile(df_sort[df_sort['MYC_group1'] == 2]['dis_to_hub_area_normalized'].tolist(), np.arange(0, 1.1, 0.1))
quantiles3 = np.quantile(df_sort[df_sort['MYC_group1'] == 3]['dis_to_hub_area_normalized'].tolist(), np.arange(0, 1.1, 0.1))
print(quantiles2)
print(quantiles3)
...
sns.barplot(data=df_sort, x='MYC_group1', y='dis_to_hub_area_normalized', order=hue_order2)
# plt.savefig('%s/%s_dis_to_hub_area_normalized.pdf' % (output_dir, figure_name))
plt.show()"""
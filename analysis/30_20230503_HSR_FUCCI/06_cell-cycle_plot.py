...
df_G1 = df[(df['ln_mean_int_red_cal'] > 5) & (df['ln_mean_int_green_cal'] < 5)].copy()
df_S = df[(df['ln_mean_int_red_cal'] < 5) & (df['ln_mean_int_green_cal'] > 5)].copy()
df_G2 = df[(df['ln_mean_int_red_cal'] > 5) & (df['ln_mean_int_green_cal'] > 5)].copy()
df_neg = df[(df['ln_mean_int_red_cal'] < 5) & (df['ln_mean_int_green_cal'] < 5)].copy()

cellcycle = []
for i in range(len(df)):
    if (df['ln_mean_int_red_cal'][i] > 5) & (df['ln_mean_int_green_cal'][i] < 5):
        cellcycle.append('G1')
    elif (df['ln_mean_int_red_cal'][i] < 5) & (df['ln_mean_int_green_cal'][i] > 5):
        cellcycle.append('S')
    elif (df['ln_mean_int_red_cal'][i] > 5) & (df['ln_mean_int_green_cal'][i] > 5):
        cellcycle.append('G2M')
    elif (df['ln_mean_int_red_cal'][i] < 5) & (df['ln_mean_int_green_cal'][i] < 5):
        cellcycle.append('neg')
    else:
        cellcycle.append('NA')
df['cellcycle'] = cellcycle
df.to_csv('%s%s_cellcycle_hoechstonly.txt' % (output_dir, sample), index=False, sep='\t')
# df_temp = df[df['cellcycle'].isin(['G1', 'S', 'G2M', 'M'])].copy().reset_index(drop=True)
# df = df_temp
...
plt.savefig('%s/%s.pdf' % (output_dir, sample))
plt.show()
...
"""feature1 = ['total_int_MYC_cal_mean_log2FC', 'total_int_nuclear_cal_mean_log2FC',
            'area_nuclear_mean_log2FC', 'mean_int_MYC_cal_mean_log2FC', 'mean_int_nuclear_cal_mean_log2FC']"""
feature1 = ['total_int_MYC_mean_log2FC', 'total_int_nuclear_mean_log2FC',
            'area_nuclear_mean_log2FC', 'mean_int_MYC_mean_log2FC', 'mean_int_nuclear_mean_log2FC']
feature2 = ['total_int_MYC_mean_log2FC', 'total_int_nuclear_mean_log2FC',
            'area_nuclear_mean_log2FC', 'mean_int_MYC_mean_log2FC', 'mean_int_nuclear_mean_log2FC']

# load data
df1 = pd.read_csv('%s%s_n%s_sum_1st.txt' % (data_dir1, exp1, n_nuclear_convex_dilation), na_values=['.'], sep='\t')
df2 = pd.read_csv('%s%s_n%s_sum_1st.txt' % (data_dir1, exp2, n_nuclear_convex_dilation), na_values=['.'], sep='\t')
...
for f in range(len(feature1)):
    ...
    limit = max(abs(df1[feature1[f]].max()), abs(df1[feature1[f]].min()), abs(df2[feature2[f]].max()), abs(df2[feature2[f]].min()))
    plt.xlim([-(limit + 0.01), (limit + 0.01)])
    plt.ylim([-(limit + 0.01), (limit + 0.01)])
    plt.savefig('%s/%s_vs_%s_%s_pw.pdf' % (output_dir, exp1, exp2, feature1[f]))
    plt.close()
    ...
    plt.xlim([-(limit + 0.01), (limit + 0.01)])
    plt.ylim([-(limit + 0.01), (limit + 0.01)])
    plt.savefig('%s/%s_vs_%s_%s_compound.pdf' % (output_dir, exp1, exp2, feature1[f]))
    plt.close()
    ...
    plt.xlim([-(limit + 0.01), (limit + 0.01)])
    plt.ylim([-(limit + 0.01), (limit + 0.01)])
    plt.savefig('%s/%s_vs_%s_%s_target.pdf' % (output_dir, exp1, exp2, feature1[f]))
    plt.close()
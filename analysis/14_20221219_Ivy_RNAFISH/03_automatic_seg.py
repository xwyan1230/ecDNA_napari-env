...
for fov in range(len(fov_lst)):
    ...
    tif.imwrite("%s%s/seg_tif/%s_%s_seg.tif" % (output_dir, sample, sample, fov_lst[fov]), img_nuclear_seg_convex)
    ...
    plt.imsave('%s%s/color_img/%s_%s_img.tiff' % (output_dir, sample, sample, fov_lst[fov]), dis.blending(viewer))
    viewer.close()

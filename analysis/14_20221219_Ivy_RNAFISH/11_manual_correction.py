...
# fov_lst = [fov_to_str(i) for i in np.arange(1, 2, 1)] + ['Mn_' + fov_to_str(i) for i in np.arange(1, 8, 1)] + [
#     'Mn_' + fov_to_str(i) for i in np.arange(9, 19, 1)]  # PC9
...
for fov in range(len(fov_lst)):
    ...
    img_nuclear_seg_convex = skio.imread("%s%s/seg_tif_manual/%s_%s_seg.tif" % (data_dir2, sample, sample, fov_lst[fov]),
                                         plugin="tifffile")
    ...
    img_RNAFISH_seg_pre = skio.imread("%s%s/seg_tif_manual/%s_%s_RNAseg.tif" % (data_dir2, sample, sample, fov_lst[fov]),
                                         plugin="tifffile")
    img_RNAFISH_seg_pre = erosion(img_RNAFISH_seg_pre, disk(3))
    img_RNAFISH_seg_pre = obj.remove_large(img_RNAFISH_seg_pre, 4)
    img_RNAFISH_seg_pre = dilation(img_RNAFISH_seg_pre, disk(2))
    img_RNAFISH_seg_pre[img_RNAFISH_seg == 1] = 1

    viewer = napari.Viewer()
    viewer.add_image(img_hoechst, blending='additive', colormap='blue', contrast_limits=[0, img_hoechst.max()])
    viewer.add_image(img_RNAFISH, blending='additive', colormap='red', contrast_limits=[0, img_RNAFISH.max()])
    viewer.add_image(img_nuclear_seg_convex, blending='additive')
    viewer.add_image(img_RNAFISH_seg_pre, blending='additive', colormap='green', contrast_limits=[0, 1])
    shapes_seg = viewer.add_shapes(name='FISH seg remove', ndim=2)
    shapes_seg1 = viewer.add_shapes(name='nuclear seg remove', ndim=2)
    ...
    tif.imwrite("%s%s/seg_tif_manual/%s_%s_RNAseg.tif" % (output_dir, sample, sample, fov_lst[fov]),
                img_RNAFISH_seg_pre)
    ...
    viewer.add_image(img_hoechst, blending='additive', colormap='blue', contrast_limits=[0, img_hoechst.max()])
    viewer.add_image(img_RNAFISH, blending='additive', colormap='red', contrast_limits=[0, img_RNAFISH.max()])
    plt.imsave('%s%s/color_img_manual/%s_%s_img.tiff' % (output_dir, sample, sample, fov_lst[fov]),
               dis.blending(viewer))
    viewer.close()

    viewer = napari.Viewer()
    viewer.add_image(img_nuclear_seg_convex, blending='additive', colormap='blue', contrast_limits=[0, 1])
    viewer.add_image(img_RNAFISH_seg_pre, blending='additive', colormap='red', contrast_limits=[0, 1])
    plt.imsave('%s%s/color_img_manual/%s_%s_seg.tiff' % (output_dir, sample, sample, fov_lst[fov]),
               dis.blending(viewer))
    viewer.close()

...
data = pd.DataFrame(columns=['nuclear', 'FOV',
                             ...
                             'RNAseg_label', 'int_r_to_edge', 'int_r_to_center', 'int_relative_r'])
...
for fov in range(len(fov_lst)):
    ...
    img_seg = skio.imread("%s%s/seg_tif_%s/%s_%s_seg.tif" % (data_dir2, sample, folder, sample, fov_lst[fov]), plugin="tifffile")
    img_RNAseg = skio.imread("%s%s/seg_tif_%s/%s_%s_RNAseg.tif" % (data_dir2, sample, folder, sample, fov_lst[fov]),
                             plugin="tifffile")
    ...
    if len(nuclear_props) > 0:
        for i in range(len(nuclear_props)):
            ...
            local_RNAseg = img_RNAseg.copy()
            local_RNAseg = local_RNAseg[position[0]:position[1], position[2]:position[3]]
            ...
            RNAseg_label = img_to_pixel_int(local_nuclear_seg, local_RNAseg)
...
data.to_csv('%s%s_radial_RNAseg_n%s_%s.txt' % (output_dir, sample, n_nuclear_convex_dilation, folder), index=False, sep='\t')
print("DONE!")
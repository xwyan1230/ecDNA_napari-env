...
for fov in range(len(fov_lst)):
    ...
    img_nuclear_seg_convex = skio.imread("%s%s/seg_tif/%s_%s_seg.tif" % (data_dir2, sample, sample, fov_lst[fov]),
                                         plugin="tifffile")
    ...
    viewer.add_image(img_RNAFISH, blending='additive', colormap='red', contrast_limits=[0, img_RNAFISH.max()])
    viewer.add_image(img_nuclear_seg_convex, blending='additive')
    viewer.add_image(img_RNAFISH_seg, blending='additive', colormap='green', contrast_limits=[0, 1])
    ...
    img_RNAFISH_seg = ima.napari_add_or_remove_obj(shapes_seg.data, 'remove', img_RNAFISH_seg)
    img_nuclear_seg_convex = ima.napari_add_or_remove_obj(shapes_seg1.data, 'remove', img_nuclear_seg_convex)
    img_nuclear_seg_convex = obj.label_resort(img_nuclear_seg_convex)
    img_RNAFISH_point = np.zeros_like(img_RNAFISH_seg)
    for j in points.data:
        img_RNAFISH_point[int(j[0]), int(j[1])] = 1
    img_RNAFISH_point = dilation(img_RNAFISH_point, disk(2))
    img_RNAFISH_seg[img_RNAFISH_point == 1] = 1

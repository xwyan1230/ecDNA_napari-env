...
parser.add_argument('-cen_r', type=int)
...
im_mask = logical_ellipse(im_test, local_size, local_size, cen_r, cen_r)
im_sampling = im_seg.copy()
im_sampling[im_mask == 1] = 0
p_list = im_sampling.flatten()
...
if not os.path.exists("%s%s_%s/FISH_seg_tif/" % (output_dir, coefficient, crange)):
    os.makedirs("%s%s_%s/FISH_seg_tif/" % (output_dir, coefficient, crange))
    os.makedirs("%s%s_%s/nuclei_seg_tif/" % (output_dir, coefficient, crange))
tif.imwrite("%s%s_%s/FISH_seg_tif/im_FISH_c%s_cr%s_%s_cp%s_r%s.tif" % (output_dir, coefficient, crange, coefficient, crange, repeat, copy_num, r), im_FISH)
tif.imwrite("%s%s_%s/nuclei_seg_tif/im_seg_c%s_cr%s_%s_cp%s_r%s.tif" % (output_dir, coefficient, crange, coefficient, crange, repeat, copy_num, r), im_seg)
...
plt.imsave('%s%s_%s/color_tif/c%s_cr%s_%s_cp%s_r%s.tiff' % (
    output_dir, coefficient, crange, coefficient, crange, repeat, copy_num, r), blending(viewer))
viewer.close()

...
for i in range(len(seps)-1):
    ...
    for j in range(len(data_sample)):
        plt.plot(x[1:], data_sample['radial_curve_DNAFISH'][j][1:], alpha=0.05, color=line_color[i])
    mean_curve, ci_lower, ci_higher = dat.mean_list(data_sample['radial_curve_DNAFISH'].tolist())
    mean_df.append(mean_curve[1:])
    plt.plot(x[1:], mean_curve[1:], color=line_color[i], label='%s, n=%s' % (seps[i+1], len(data_sample)))
plt.xlabel('relative_r')
plt.ylabel('radial_curve')
plt.ylim([0, 15])
plt.legend(loc=2)
plt.savefig('%s%s_radial_curve_wo_center.pdf' % (output_dir, sample))
plt.show()

# heatmap
df = pd.DataFrame(mean_df)
df.columns = ['%.3f' % elem for elem in x][1:]
df.index = seps[1:]
plt.subplots(figsize=(12, 9))
sns.heatmap(df, cbar=0, linewidths=2, square=True, cmap='coolwarm')
plt.savefig('%s%s_radial_heatmap_wo_center.pdf' % (output_dir, sample))
plt.show()
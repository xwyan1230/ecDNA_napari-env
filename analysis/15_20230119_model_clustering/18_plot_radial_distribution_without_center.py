...
for i in range(len(samples)):
    ...
    for j in range(len(data_sample)):
        plt.plot(x[1:], data_sample['radial_curve_DNAFISH'][j][1:], alpha=0.01, color=line_color[i])
    mean_curve, ci_lower, ci_higher = dat.mean_list(data_sample['radial_curve_DNAFISH'].tolist())
    mean_df.append(mean_curve[1:])
    plt.plot(x[1:], mean_curve[1:], color=line_color[i], label='%s, n=%s' % (coefficient, len(data_sample)))
plt.xlabel('relative_r')
plt.ylabel('radial_curve')
plt.ylim([0, 20])
plt.legend(loc=2)
plt.savefig('%sradial_curve_wo_center.pdf' % output_dir)
plt.show()
...
plt.savefig('%sradial_heatmap_wo_center.pdf' % output_dir)
plt.show()
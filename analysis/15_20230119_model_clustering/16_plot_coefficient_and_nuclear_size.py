...
interval = 5
copy_nums = np.arange(50, 101, interval)
print(copy_nums)
...
for j in range(len(samples)):
    ...
    for i in range(len(copy_nums)-1):
        ...
        data_sample = data[(data['r'] > copy_nums[i]) & (data['r'] <= copy_nums[i+1])].copy().reset_index(drop=True)
...
plt.savefig('%s2d_above_area_vmax10.pdf' % output_dir)
plt.show()

...
# INPUT PARAMETERS
# file info
master_folder = "/Users/xwyan/Dropbox/LAB/ChangLab/Projects/Data/20230213_analysis_data-inconsistency/"
data_dir = "%stxt/20220822_sp8_POLR3Dko_JunDMandHSR_singleZ/" % master_folder
output_dir = "%sfigures/20220822_sp8_POLR3Dko_JunDMandHSR_singleZ/" % master_folder

samples = ['DM_Jun_Ctrl1', 'DM_Jun_POLR3Dko', 'DM_Jun_Ctrl2']
line_color = [(0.30, 0.30, 0.30), (0.85, 0.35, 0.25),  (0.30, 0.70, 0.70)]
sns.set_palette(sns.color_palette(line_color))

data = pd.DataFrame()
for i in samples:
    df = pd.read_csv(("%s%s_v1.txt" % (data_dir, i)), na_values=['.'], sep='\t')
...
data_filter = data[(data['dis_to_hub_area_v2_normalized'] < 1.2) & (data['total_area_ecDNA_sqrt_normalized'] > 0.23)].copy().reset_index(drop=True)
data = data_filter

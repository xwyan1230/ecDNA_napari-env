...
feature_lst = ['n', 'averageD_mean', 'averageD_std', 'averageD_0.5', 'area_nuclear', 'n_ecDNA']
row_lst = ['B', 'C', 'D', 'E', 'F', 'G']
...
for feature in feature_lst:
    ...
    if 'averageD' in feature:
        vmax = 1
    elif feature == 'n_ecDNA':
        vmax = 10
    elif feature == 'area_nuclear':
        vmax = 30000
    else:
        vmax = df_feature.values.max()
    ax1 = sns.heatmap(df_feature, cbar=0, linewidths=2, vmax=vmax, vmin=0, square=True, cmap='coolwarm', annot=True, fmt='.2f')
    if not os.path.exists("%s/heatmap/" % output_dir):
        os.makedirs("%s/heatmap/" % output_dir)
    plt.savefig('%s/heatmap/%s_scale.pdf' % (output_dir, feature))
    plt.close()

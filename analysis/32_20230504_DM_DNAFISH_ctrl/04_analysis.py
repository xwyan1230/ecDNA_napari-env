...
data = pd.DataFrame(columns=['well', 'nuclear', 'FOV', 'n_nuclear_convex_dilation',
                             ...
                             'dis_to_hub_area',
                             'radial_curve_nuclear', 'radial_curve_DNAFISH', 'radial_curve_DNAFISH_seg',
...
if not os.path.exists("%s/txt_full/" % output_dir):
    os.makedirs("%s/txt_full/" % output_dir)
data.to_csv('%s/txt_full/%s_n%s.txt' % (output_dir, sample, n_nuclear_convex_dilation), index=False, sep='\t')

...
for i in range(n_img):
    ...
    """IF_mask = np.zeros_like(img_nuclear_seg)
    for j in range(np.max(img_nuclear_seg)):
        temp = np.zeros_like(img_nuclear_seg)
        temp[img_nuclear_seg == j+1] = 1
        temp = erosion(temp, disk(3))
        IF_mask[temp == 1] = j+1"""

    blue_props = regionprops(img_nuclear_seg, img_hoechst)
    print(len(blue_props))
    ...
    # viewer.add_image(IF_mask, blending='additive', colormap='red', contrast_limits=[0, np.max(img_nuclear_seg)])

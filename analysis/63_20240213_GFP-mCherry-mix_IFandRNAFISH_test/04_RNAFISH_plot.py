...
plt.savefig('%s/%s/violin_log10_RNAFISH_2500.pdf' % (output_dir, folder))
plt.show()"""

"""for sample in samples:
    plt.subplots(figsize=(6, 6))
    sns.scatterplot(data=df[df['sample'] == sample], x='log10_mCherry', y='log10_GFP', s=5, alpha=0.5, color='r')
    sns.scatterplot(data=df[df['sample'] == 'DM_2500'], x='log10_mCherry', y='log10_GFP', s=5, alpha=0.05)
    plt.xlim([2.2, 5.0])
    plt.ylim([3.0, 5.0])
...
sns.violinplot(data=df1[df1['group'].isin(['DM', 'mCherry'])], x='group', y='log10_RNAFISH', alpha=0.5)
plt.savefig('%s/%s/violin_log10_RNAFISH_%s.pdf' % (output_dir, folder, x))
plt.show()

"""x = 'GFP(M)andmCherry_2500'
df1 = pd.read_csv('%s%s/txt/%s.txt' % (data_dir, folder, x), na_values=['.'], sep='\t')
df1['log10_GFP'] = np.log10(df1['GFP'])
df1['log10_mCherry'] = np.log10(df1['mCherry'])
df1['log10_RNAFISH'] = np.log10(df1['RNAFISH'])
...
for i in range(len(df1)):
    if (df1['log10_mCherry'][i] > 2.9) & (df1['log10_GFP'][i] < 3.6):
        group_lst.append('mCherry')
    elif ((df1['log10_mCherry'][i] < 2.9) & (df1['log10_GFP'][i] > 3.5)) | ((df1['log10_mCherry'][i] < 2.6) & (df1['log10_GFP'][i] <= 3.5)):

...
for i in range(len(test_lst)):
    ...
    sns.scatterplot(data=data_screen, x=feature, y='log10_fov_hoechst', alpha=0.8, s=size_lst[i], color=MK1775_colors[i])
    sns.scatterplot(data=data_screen[data_screen['treatment'] == 'DMSO'], x=feature, y='log10_fov_hoechst', color=ctrl_colors[i], alpha=0.8, s=size_lst[i])
plt.xlim([0, 3])
plt.ylim([8, 11])
plt.savefig('%s/%s_HSR-DM_hoechst_MK1775_update1_hc.pdf' % (output_dir, folder))
plt.show()
...
for i in range(len(test_lst)):
    ...
    sns.scatterplot(data=data_screen, x=feature, y='log10_fov_hoechst', alpha=0.8, s=size_lst[i], color=rainboo_colors[i])
plt.xlim([0, 3])
plt.ylim([8, 11])
plt.savefig('%s/%s_HSR-DM_hoechst_density_update1_hc.pdf' % (output_dir, folder))
plt.show()

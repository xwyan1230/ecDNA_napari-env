...
folder = 'IF'
samples1 = ['Ctrl', 'GFP', 'mCherry', 'mix_96', 'mix_384']
...
plt.savefig('%s/%s/violin_log10_IF.pdf' % (output_dir, folder))
plt.show()"""

"""for sample in samples1:
    df1 = pd.read_csv('%s%s/txt/%s.txt' % (data_dir, folder, sample), na_values=['.'], sep='\t')
    df2 = pd.read_csv('%s%s/txt/Ctrl.txt' % (data_dir, folder), na_values=['.'], sep='\t')
    df1['log10_GFP'] = np.log10(df1['GFP'])
    df1['log10_mCherry'] = np.log10(df1['mCherry'])
    df2['log10_GFP'] = np.log10(df2['GFP'])
    df2['log10_mCherry'] = np.log10(df2['mCherry'])
    plt.subplots(figsize=(9, 6))
    sns.scatterplot(data=df1, x='log10_mCherry', y='log10_GFP', s=5, alpha=0.5, color='r')
    sns.scatterplot(data=df2, x='log10_mCherry', y='log10_GFP', s=5, alpha=0.5)
    plt.xlim([2.2, 4.8])
    plt.ylim([3, 4.8])
...
plt.xlim([3.3, 4.8])
plt.ylim([3.1, 4.8])
plt.savefig('%s/%s/int_vs_IF_%s.pdf' % (output_dir, folder, x))
plt.show()
...
plt.xlim([3.3, 4.8])
plt.ylim([3.1, 4.8])
plt.savefig('%s/%s/GFPint_vs_IF_%s.pdf' % (output_dir, folder, x))
plt.show()
...
plt.xlim([3.3, 4.8])
plt.ylim([3.1, 4.8])
plt.savefig('%s/%s/mCherryint_vs_IF_%s.pdf' % (output_dir, folder, x))
plt.show()